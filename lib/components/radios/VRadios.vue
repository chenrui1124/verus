...
<template>
  <div :style="{ width }" class="flex flex-col gap-2">
    <label
      ...
      :key="index"
      class="group/v-radio relative box-border inline-flex h-12 min-w-12 cursor-pointer select-none items-center justify-center gap-4 rounded-v2 border-v1 border-solid border-otl px-4 text-on-bsc outline-transparent transition-all duration-300 v-outline-none hover:bg-pri/8 has-[:checked]:border-pri has-[:checked]:bg-pri-ctr has-[:checked]:text-pri has-[:focus-visible]:v-outline"
    >
